...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	 ...
	 else
	{
		...
		$qry = "select * from Blood_rgst where Username='$username'";
		$rsl = $conn->query($qry);
		...
		{
			$qry = "insert into blood_rgst(Username,Password,Phone_number,Address)
					values('$username', '$password', '$number', '$address');";
			$res = $conn->query($qry);
			
			header('location: Blood_donate.php');
...
<center><h1><b><i>Registration  Form  For Blood Donation Service </i></b></h1></center>
